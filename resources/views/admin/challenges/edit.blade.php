...
@section('title', $chall->title.' - Chỉnh sửa bài Viết')
@section('customcss')
                ...
                <div class="col-md-8 ps-1 pe-1">
                    <div class="card border-primary mb-2">
                        <div class="card-header bg-light">
                            <p class="m-0">
                                <a href="{{route('ql_baiviet.index')}}">Danh sách Challend</a> /
                                <a href="{{route('ql_baiviet.show', $chall->id)}}">{{$chall->title}}</a> /
                            </p>
                            <h3 class="inline">Chỉnh sửa Challend</h3>
                        </div>
                        <div class="card-body">
                            {{ csrf_field() }}
                            <div class="form-group">
                                <label for="title">Tên Challend</label>
                                <input type="text" value="{{$chall->title}}" class="form-control is-valid edit"
                                    name="title" id="title" aria-describedby="titleHelp"
                                    placeholder="vd: Tài liệu Nhập môn Mạng máy tính" idPost="{{ $chall->id }}"
                                    required>
                                ...
                                <label class="form-check-label">
                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox" class="custom-control-input" name="status" id="status"
                                            value="1" {{ ($chall->status == 1) ? 'checked' : '' }}>
                                        <label class="custom-control-label" for="status">Công khai activeChallend
                                            này</label>
                                    </div>
                            </div>
                        ...
                        <div class="modal-dialog mt-5 pt-5" role="document">
                            <div class="modal-content border-danger">
                                ...
                                <div class="modal-body">
                                    ...
                                    Tất cả <strong>Tài liệu</strong> nằm trong Challend
                                    <strong>{{ $chall->title }}</strong> sẽ được chuyển qua <strong>Thùng rác</strong>!
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Hủy</button>
                                    <button type="button" id="deleteBaiviet" name="delete" class="btn btn-danger"><i
                                            class="fa fa-trash pe-1"></i>Xóa Challend</button>
                                </div>
                            </div>
                        </div>
                    </div>
